...
evaluated against an over-(G+0.5) line only — the live-relevant case. It "wins"
if at least one more goal is scored within DEFAULT_GOAL_WINDOW minutes of m
(or, trivially, if the line was already cleared, which we exclude by the
goals == floor(line) filter). Markets where G != floor(line) are skipped.
...
def _win_series(triggers: pd.DataFrame, line: float, window: int) -> pd.Series:
    col = f"next_goal_within_{window}"
    if col not in triggers.columns:  # fall back to nearest available horizon
        col = "next_goal_within_15"
    already_cleared = triggers["goals_total_at_trigger"] > line
    return (already_cleared | (triggers[col] == 1)).astype(int)
...
def _evaluate(triggers: pd.DataFrame, market: str, window: int) -> dict:
    ...
    if n == 0:
        return out
    wins = _win_series(sub, line, window)
    ...
    if odds_col in sub.columns and sub[odds_col].notna().any():
        ...
        priced_wins = _win_series(priced, line, window)
...
def run_backtest(conn=None, *, window: int | None = None, write: bool = True) -> pd.DataFrame:
    own = conn is None
    if own:
        conn = get_conn()
    window = window or config.DEFAULT_GOAL_WINDOW
    try:
        ...
        for signal_type, threshold, market, min_m, max_m in _grid():
            ...
            stats = _evaluate(sel, market, window)
            rows.append({
